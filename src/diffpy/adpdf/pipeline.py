...
class KKCalcAdapter:
    ...
    def f1_from_f2(self, energy_eval_eV: np.ndarray, near_edge_f2_vals: Optional[np.ndarray],
                   merge_points: Optional[Tuple[float, float]] = None,
                   add_background: bool = False, fix_distortions: bool = False) -> Tuple[np.ndarray, np.ndarray, float]:
...
class EdgeModel(_BaseEdgeModel):
    ...
    def fp(self, E: np.ndarray, fpp: Optional[np.ndarray] = None, return_f1: bool = False,
           merge_points: Optional[Tuple[float, float]] = None, add_background: bool = False,
           fix_distortions: bool = False) -> np.ndarray:
        E_eV = self.to_eV(E)
        E_eval = E_eV + float(self.params.dE_eV)
        if fpp is None: fpp = self.fpp(E_eV)
        f1_vals, f2_vals, f1_inf = self.kk.f1_from_f2(
            energy_eval_eV=E_eval, near_edge_f2_vals=fpp, merge_points=merge_points,
            add_background=add_background, fix_distortions=fix_distortions
        )
...
def _as_1d(a: np.ndarray) -> np.ndarray: return np.asarray(a).ravel()
def _trapz(x: np.ndarray, y: np.ndarray) -> float: return float(np.trapezoid(y, x))

def _safe(a: np.ndarray, name: str) -> np.ndarray:
    if not np.all(np.isfinite(a)):
        raise ValueError(f"{name} has non-finite entries at {np.where(~np.isfinite(a))}")
    return a
...
def _ensure_2d(x: np.ndarray, ne: int, nq: int, name: str) -> np.ndarray:
    ...
    if x.ndim == 0: return np.full((ne, nq), float(x))
    if x.ndim == 1:
        if x.size == ne: return x[:, None] * np.ones((1, nq))
        if x.size == nq: return np.ones((ne, 1)) * x[None, :]
        raise ValueError(f"{name} 1D mismatch")
    if x.shape != (ne, nq): raise ValueError(f"{name} shape mismatch")
    return x
...
def _compute_scattering(elements: List[str], comp: Dict[str, float], alpha: str, Q: np.ndarray,
                        E_eV: np.ndarray, edge_model: Any, fp_offset: float = 0.0):
    ...
    fmean_real, fmean_abs2, f_not_sum = np.zeros((ne, nq), float), np.zeros((ne, nq), float), np.zeros((ne, nq), float)
    for el in elements:
        c = float(comp[el])
        fmean_real += c * fr_weights[el]
        fmean_abs2 += c * f2_weights[el]
        if el != alpha: f_not_sum += c * fr_weights[el]
    return fr_weights, f0_map, fmean_real, fmean_abs2, fmean_abs2, f_not_sum

def _weights_alpha_not(comp: Dict[str, float], alpha: str, fr_weights: Dict[str, np.ndarray],
                       fmean_real: np.ndarray, f_not_sum: np.ndarray):
...
def _alpha_beta_from_fpp(fpp_E: np.ndarray, alpha_C: float, alpha_D: float, beta_C: float, beta_D: float):
    return np.clip(alpha_C + alpha_D * fpp_E, 1e-6, None), beta_C + beta_D * fpp_E

def _build_S_from_raw(I_raw: np.ndarray, Q: np.ndarray, E_eV: np.ndarray, alpha_e: np.ndarray, beta_f_e: np.ndarray,
                      fmean_real: np.ndarray, fself: np.ndarray, rpoly: float):
    ...
    poly_weights = [0.5, 0.5] if porderlo == porderhi else [porderhi - porder, porder - porderlo]
...
def _solve_two_channel_column(Aj: np.ndarray, yj: np.ndarray, ridge: float, anchor: float):
    ATA = Aj.T @ Aj + ridge*np.eye(2) + anchor*np.eye(2)
    ATy = Aj.T @ yj + anchor*np.array([[1.0],[1.0]])
    try: sol = npl.solve(ATA, ATy)
    except npl.LinAlgError: sol = npl.lstsq(ATA, ATy, rcond=None)[0]
    return float(sol[0,0]), float(sol[1,0])

def _reconstruct_Salpha_Snot(S_meas: np.ndarray, a: np.ndarray, b: np.ndarray, Q: np.ndarray, ridge: float):
    ...
    for j in range(nq):
        ...
        if anchor > 0.0: bad_cols += 1
        Sa[j], Sb[j] = _solve_two_channel_column(Aj, yj, ridge, anchor)
    return Sa, Sb, {"bad_Q_columns": bad_cols}

def _S_to_G(Q: np.ndarray, S: np.ndarray, r: np.ndarray, window: str, kbeta: float) -> np.ndarray:
...
def save_initial_edge_plots(E_eV: np.ndarray, edge_model: EdgeModel, cfg: RefinementConfig):
    fpp_init = edge_model.fpp(E_eV)
    fp_init = edge_model.fp(E_eV)
    fp_chantler = xraydb.f1_chantler(edge_model.params.element, edge_model.to_eV(E_eV) + edge_model.params.dE_eV)
    
    fig, ax = plt.subplots(1, 2, figsize=(10, 4))
    ax[0].plot(E_eV / 1000, fpp_init, label="f'' (Model/Chantler)")
    ax[0].set_xlabel("Energy (keV)"); ax[0].set_title("Initial f''"); ax[0].legend()
    
    ax[1].plot(E_eV / 1000, fp_init, label="f' (Model)")
    ax[1].plot(E_eV / 1000, fp_chantler, "--", label="f' (Chantler Tabulated)")
    ax[1].set_xlabel("Energy (keV)"); ax[1].set_title("Initial f'"); ax[1].legend()
    
    plt.tight_layout()
    plt.savefig(os.path.join(cfg.fig_dir, "fpp_fp_initial.png"), dpi=150)
    plt.close()

def _plot_frame(path: str, E_eV: np.ndarray, edge_model: Any, r: np.ndarray, out: Dict[str,np.ndarray],
                Q_full: np.ndarray, iter_idx: int, hist: List[RefinementHistory], fp_offset: float) -> None:
    q = np.asarray(out.get("Q", Q_full))
    Sa, Sb, St = np.asarray(out["S_alpha"]), np.asarray(out["S_not"]), np.asarray(out["S_tot"])
    r_plot = np.asarray(r)
    Ga, Gn, Gt = np.asarray(out["G_alpha"]), np.asarray(out["G_not"]), np.asarray(out["G_tot"])
    ...
    ax1.plot(E/1e3, fpp, label="f'' (model)"); ax1.legend(loc="upper left")

    ax2 = plt.subplot(4,2,2)
    ax2.plot(E/1e3, fp, label="f' (KK from model) + fp_offset"); ax2.legend(loc="upper left")

    ax = plt.subplot(4,1,2)
    ax.plot(r_plot, Ga, 'b-', label="G_alpha"); ax.plot(r_plot, Gn, 'g-', label="G_not")
    ax.plot(r_plot, Gt, "r-", label="G_tot"); ax.legend(loc="upper right")

    ax = plt.subplot(4,1,3)
    ax.plot(q, Sa, 'b-', label="S_alpha", linewidth=.5); ax.plot(q, Sb, 'g-', label="S_not", linewidth=.5)
    ax.plot(q, St, "r-", label="S_tot", linewidth=.5); ax.plot(q, np.ones_like(q), 'k-', linewidth=0.1)
    ax.legend(loc="upper left")

    ax = plt.subplot(4,1,4)
    its = np.arange(len(hist)); vals = np.array([h.obj for h in hist], float)
    ax.plot(its, vals, "-o"); ax.set_yscale('log')
    p = hist[-1].params
...
def save_results(out: Dict[str, np.ndarray], cfg: RefinementConfig):
    ...
    r = out["r"]
    np.savetxt(os.path.join(cfg.out_dir, "G_alpha.cgr"), np.column_stack([r, out["G_alpha"]]), header="r G_alpha")
    np.savetxt(os.path.join(cfg.out_dir, "G_not.cgr"), np.column_stack([r, out["G_not"]]), header="r G_not")
    np.savetxt(os.path.join(cfg.out_dir, "G_tot.cgr"), np.column_stack([r, out["G_tot"]]), header="r G_tot")
...
def _objective(Sa: np.ndarray, Sb: np.ndarray, Q: np.ndarray, hiQ_frac: float, anchor_lambda: float) -> float:
...
def reconstruct_partials_and_pdfs(I_raw, Q, E_eV, elements, comp, alpha, edge_model, alpha_e, beta_e,
                                  rpoly, ridge, r, Qmin, Qmax, window="lorch", kaiser_beta=6.0, fp_offset=0.0):
    ...
    S_meas_all, beta_o_all = _build_S_from_raw(I_raw, Q, E, alpha_e, beta_e, fmean_real, fself, rpoly)
    S_meas = S_meas_all[:, mask_Q]; a = a[:, mask_Q]; b = b[:, mask_Q]
    ...
    out = {"Q": Q_use, "r": r, "S_alpha": Sa, "S_not": Sb, "S_tot": S_tot,
           "G_alpha": G_alpha, "G_not": G_not, "G_tot": G_tot}
    return out, {"weights": wb, "condinfo": condinfo, "beta_o": beta_o_all}

def run_adpdf_with_refinement(I_raw, Q, E_eV, elements, comp, alpha, edge_model,
                              alpha_C=1.0, alpha_D=0.0, beta_C=0.0, beta_D=0.0,
                              fp_offset=0.0, cfg=None):
    ...
    def compute():
        ...
        out, diag = reconstruct_partials_and_pdfs(
            I_raw, Q, E, elements, comp, alpha, edge_model, alpha_e, beta_e,
            rpoly=rpoly, ridge=cfg.ridge, r=r, Qmin=cfg.Qmin, Qmax=Qmax,
            window=cfg.window, kaiser_beta=cfg.kaiser_beta, fp_offset=fp_offset,
        )
        return out, diag, _objective(out["S_alpha"], out["S_not"], out["Q"], cfg.hiQ_frac, cfg.hiQ_anchor_lambda)

    out, diag, obj = compute()
    hist = [RefinementHistory(0, obj, {
    ...
    _plot_frame(os.path.join(cfg.frame_dir, f"frame_{0:03d}.png"), E, edge_model, r, out, Q, 0, hist, fp_offset)

    steps = dict(rpoly=cfg.step_rpoly, dE_eV=cfg.step_dE_eV, scale=cfg.step_scale,
                 fpp_offset=cfg.step_fpp_offset, fp_offset=cfg.step_fp_offset,
                 broaden_fwhm_eV=cfg.step_broaden_fwhm, alpha_C=cfg.step_alphaC,
                 alpha_D=cfg.step_alphaD, beta_C=cfg.step_betaC, beta_D=cfg.step_betaD)
    ...
    def tweak(name: str, d: float):
        ...
        elif name == "broaden_fwhm_eV":p.broaden_fwhm_eV = float(np.clip(p.broaden_fwhm_eV + d, cfg.min_broaden_fwhm_eV, cfg.max_broaden_fwhm_eV))
    ...
    for it in range(1, cfg.n_outer+1):
        ...
        for name in ["rpoly", "dE_eV","scale","fpp_offset","fp_offset","broaden_fwhm_eV",
            ...
            for sgn in (+1,-1):
                ...
                if obj2 < best_obj:
                    ...
                    param_improved = True; cycle_improved = True
                else:
                    tweak(name, -sgn*steps[name])
        ...
        if (it % cfg.plot_every)==0 or it==cfg.n_outer:
            _plot_frame(os.path.join(cfg.frame_dir, f"frame_{it:03d}.png"), E, edge_model, r, out, Q, it, hist, fp_offset)

        rel_change = (start_obj - best_obj) / max(start_obj, 1e-12)
        if rel_change < cfg.tol_obj:
            print(f"Converged! Relative change ({rel_change:.2e}) is below tolerance.")
            break
